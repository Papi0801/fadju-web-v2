...
const RendezVousPage: React.FC = () => {
    ...
    const handleStatusChange = async (rdvId: string, newStatus: 'confirme' | 'annule') => {
        ...
        try {
            ...
            toast.success(`Rendez-vous ${((newStatus as any) === 'confirme' || (newStatus as any) === 'confirmee') ? 'confirmé' : 'annulé'} avec succès`);
            fetchData();
        } catch (error) {
    ...
    const stats = {
        ...
        confirmes: rendezVous.filter(rdv => (rdv.statut as any) === 'confirmee' || (rdv.statut as any) === 'confirme').length,
        annules: rendezVous.filter(rdv => (rdv.statut as any) === 'annulee' || (rdv.statut as any) === 'annule').length,
        reportes: rendezVous.filter(rdv => rdv.statut === 'reportee').length,
        termines: rendezVous.filter(rdv => (rdv.statut as any) === 'terminee' || (rdv.statut as any) === 'termine').length,
    };
    ...
    return (
        <DashboardLayout>
            ...
            >
                ...
                {(user as any)?.etablissement_statut === 'en_attente' && (
                    <Card
                ...
                >
                    {patientModal.patient && (
                        <div className="space-y-6">
                            ...
                            <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm sm:text-base font-medium break-all">{(patientModal.patient as any).personne_a_contacter}</p>
                                    </div>
                                    <div className="sm:col-span-2">
                                    ...
                                    <div className="sm:col-span-2">
                                        ...
                                        <p className="text-sm sm:text-base font-medium">
                                            {(patientModal.patient as any).maladie_chronique || 'Aucune maladie chronique'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            {(patientModal.patient as any).notes && (
                                <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
                                    <h3 className="font-semibold text-lg mb-3 flex items-center space-x-2">
                                        <FileText className="w-5 h-5 text-blue-500" />
                                        <span>Notes complémentaires</span>
                                    </h3>
                                    <p className="text-sm">{(patientModal.patient as any).notes}</p>
                                </div>
                            )}
                        </div>
                    )}
