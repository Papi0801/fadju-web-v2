...
const ConsultationsPage: React.FC = () => {
    ...
    const handleStatusChange = async (rdvId: string, newStatus: 'terminee' | 'annulee') => {
        try {
            if (newStatus === 'terminee') {
                ...
                if (!rdv || !rdv.date_rendez_vous) return;
    ...
    const handleFinishConsultation = async () => {
        ...
        try {
            ...
            if (!selectedRdv.date_rendez_vous) {
                toast.error('Erreur: Date du rendez-vous manquante');
                return;
            }
    ...
    const groupByDate = (rdvList: RendezVous[]) => {
        const grouped = rdvList.reduce((groups, rdv) => {
            ...
            if (!rdv.date_rendez_vous) {
                console.warn('RDV sans date_rendez_vous:', rdv);
                return groups;
            }
        ...
        return sortedDates.map(date => ({
            ...
            rdvs: grouped[date].sort((a, b) => {
                if (!a.date_rendez_vous || !b.date_rendez_vous) return 0;
                return a.date_rendez_vous.toDate().getTime() - b.date_rendez_vous.toDate().getTime();
            })
        }));
    };
